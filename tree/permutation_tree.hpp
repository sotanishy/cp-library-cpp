...
public:
    ...
    explicit PermutationTree(const std::vector<int>& p) {
        LazySegmentTree<MinMonoid, AddMonoid, act> seg(std::vector<int>(p.size()));  // seg.fold(l, r) == min_{l <= j < r} { max(p[j:i]) - min(p[j:i]) - (i - j) }
        std::vector<int> st_max = {-1}, st_min = {-1}, st;

        for (int i = 0; i < (int) p.size(); ++i) {
            while (st_max.back() >= 0 && p[st_max.back()] < p[i]) {
                seg.update(st_max[st_max.size() - 2] + 1, st_max.back() + 1, p[i] - p[st_max.back()]);
                st_max.pop_back();
            }
            st_max.push_back(i);

            while (st_min.back() >= 0 && p[st_min.back()] > p[i]) {
                seg.update(st_min[st_min.size() - 2] + 1, st_min.back() + 1, -(p[i] - p[st_min.back()]));
                st_min.pop_back();
            }
            st_min.push_back(i);

            nodes.push_back({Leaf, i, i + 1, p[i], p[i] + 1, std::vector<int>{}});
            int v = nodes.size() - 1;  // index of the current node

            while (true) {
                NodeType join_tp = None;
                if (!st.empty() && nodes[st.back()].ub == nodes[v].lb) join_tp = JoinAsc;
                if (!st.empty() && nodes[st.back()].lb == nodes[v].ub) join_tp = JoinDesc;
                ...
                if (!st.empty() && join_tp != None) {
                    ...
                    if (join_tp == nodes[st.back()].tp) {
                        ...
                        int u = st.back();
                        nodes.push_back({join_tp, nodes[u].l, nodes[u].r, nodes[u].lb, nodes[u].ub, {u}});
                        st.pop_back();
                        add_child(nodes.size() - 1, v);
                        v = nodes.size() - 1;
                    }
                } else if (seg.fold(0, i + 1 - (nodes[v].r - nodes[v].l)) == 0) {
                    // cut
                    int l = nodes[v].l, r = nodes[v].r, lb = nodes[v].lb, ub = nodes[v].ub;
                    nodes.push_back({Cut, l, r, lb, ub, {v}});
                    v = nodes.size() - 1;
                    do {
                        add_child(v, st.back());
                        st.pop_back();
                    } while (nodes[v].ub - nodes[v].lb != nodes[v].r - nodes[v].l);
                    std::reverse(nodes[v].ch.begin(), nodes[v].ch.end());
                } else {
                    break;
                }
            }

...
data:
  ...
  bundledCode: "#line 2 \"math/interpolation.cpp\"\n#include <vector>\n#line 2 \"\
    ...
    \ 0; i < (int) x.size(); ++i) prod[n + i] = {-x[i], 1};\n    for (int i = n -\
    \ 1; i > 0; --i) prod[i] = prod[2 * i] * prod[2 * i + 1];\n    auto f = prod[1].diff();\n\
  ...
  code: "#pragma once\n#include <vector>\n#include \"polynomial.cpp\"\n\ntemplate\
    ...
    \ y) {\n    assert(x.size() == y.size());\n    int n = 1;\n    while (n < (int)\
    \ x.size()) n <<= 1;\n    std::vector<Polynomial<T>> prod(2 * n, {1});\n    for\
    \ (int i = 0; i < (int) x.size(); ++i) prod[n + i] = {-x[i], 1};\n    for (int\
    \ i = n - 1; i > 0; --i) prod[i] = prod[2 * i] * prod[2 * i + 1];\n    auto f\
    \ = prod[1].diff();\n    std::vector<Polynomial<T>> poly(2 * n, {0});\n    for\
    \ (int i = 0; i < (int) x.size(); ++i) poly[n + i] = {y[i] / f(x[i])};\n    for\
    \ (int i = n - 1; i > 0; --i) poly[i] = poly[2 * i] * prod[2 * i + 1] + poly[2\
    \ * i + 1] * prod[2 * i];\n    return poly[1];\n}"
  dependsOn:
  - math/polynomial.cpp
  - math/ntt.cpp
  ...
  requiredBy: []
  timestamp: '2021-01-17 17:28:02+09:00'

...
data:
  ...
  bundledCode: "#line 2 \"data-structure/bst/lazy_treap.cpp\"\n#include <cassert>\n\
    ...
    \ update(int l,int r,const E &x){update(root,l,r,x);}\n\n    void reverse(int\
  ...
  code: "#pragma once\n#include <cassert>\n#include <memory>\n#include <random>\n\
    ...
    \ std::move(s.second));\n        root = join(join(std::move(s.first), new Node(x)),\
    \ std::move(s.second));\n    }\n\n    void erase(int k) {\n        auto p = split(std::move(root),\
